...
globals [
  ...
  ;; --- novas variáveis de controle manual/automático ---
  modo-automatico?                  ; verdadeiro = automático
  abertura-manual         ; valor manual (0–1)
]
...
to setup
  ...
  set capacidade-maxima 357000000    ; 357000000 de m3 (ajustar conforme dados reais)
  ...
  set modo-automatico? false
  set abertura-comportas-manual 0.5
...
to setup-ambiente-gis
  ...
  ask patches with [pycor < -10 and tipo-area = "indefinido"] [
    set tipo-area "rio-jusante"
    set pcolor blue - 1
  ]

  ; Criar zona urbana (extremo sul)
  ask patches with [pycor < -20 and tipo-area = "rio-jusante"] [
...
to ajustar-comportas
  ...
  ifelse modo-automatico? [
    ...
    ;; ----- MODO MANUAL -----
    set abertura-comportas abertura-comportas-manual
    ask comportas [
      set color blue
      set size 1.5 + (abertura-comportas * 1.5)
    ]
  ]
end
...
to abrir-comporta
  set abertura-comportas-manual min (list 1 (abertura-comportas-manual + 0.1))
end

to fechar-comporta
  set abertura-comportas-manual max (list 0 (abertura-comportas-manual - 0.1))
end
